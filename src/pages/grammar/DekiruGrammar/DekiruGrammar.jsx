...
const MemorizeCard = ({ item }) => {
  ...
  return (
    <div onClick={() => setRevealed(!revealed)} className="bg-white border border-slate-100 rounded-3xl p-8 hover:border-slate-300 transition-all cursor-pointer shadow-sm hover:shadow-md">
       <h3 className="text-3xl font-extrabold text-slate-900 font-kanji mb-3 tracking-tight">{item.pattern}</h3>
       {revealed ? (
         <div className="animate-in fade-in duration-300 pt-6 border-t border-slate-50 mt-4">
           <p className="text-lg md:text-xl font-bold text-slate-800 italic mb-2 leading-snug">{item.meaning}</p>
           <p className="text-base font-semibold text-slate-400 mb-6">{item.explanation}</p>
           {item.examples && item.examples.length > 0 && (
             <div className="space-y-3 bg-slate-50/70 p-6 rounded-2xl border border-slate-50">
               {item.examples.slice(0,1).map((ex, i) => (
                 <div key={i} className="flex items-center justify-between gap-6">
                   <div className="space-y-2">
                     <p className="text-xl md:text-2xl font-bold text-slate-900 font-kanji leading-relaxed">{ex.jp}</p>
                     <p className="text-sm md:text-base text-slate-600 font-semibold italic">{ex.vn}</p>
                   </div>
                   <button
                     onClick={(e) => {
                       e.preventDefault();
                     ...
                     className="p-3 bg-white hover:bg-slate-900 hover:text-white rounded-2xl text-slate-400 shadow-md border border-slate-100 transition-all flex items-center justify-center shrink-0 hover:scale-110"
                     title="Nghe phát âm"
                   >
         ...
         <div className="text-xs font-black text-slate-300 uppercase tracking-widest mt-4 flex items-center gap-2">
           Nhấn để xem nghĩa
         </div>
       )}
    </div>
