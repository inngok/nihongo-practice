...
export default function DekiruVocab() {
  ...
  const currentData = useMemo(() => {
    return dekiruData.find(s => s.id === activeSection) || dekiruData[0];
  }, [activeSection]);
            ...
            <div className="space-y-6 flex-grow">
               <div className="flex flex-col gap-4">
                ...
                <div className="flex gap-2 flex-wrap text-nowrap">
                  {dekiruData.map(section => (
                    <button
                      key={section.id}
                      onClick={() => { setActiveSection(section.id); setViewMode('list'); }}
                      className={`px-4 py-1.5 border ${activeSection === section.id ? 'bg-black text-white border-black shadow-sm' : 'border-slate-100 text-slate-400 hover:border-black hover:text-black'} text-[10px] font-bold transition-all duration-300 rounded-lg`}
                    >
               ...
               <div className="border-l-4 border-black pl-6 py-2 animate-in slide-in-from-left-4 duration-500">
                <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter leading-tight italic uppercase">
                  {currentData.title}
                </h1>
                <p className="text-xs md:text-sm text-slate-400 font-medium italic mt-1 uppercase tracking-widest leading-relaxed">
                  Dekiru Nihongo - {currentData.japanese}
                </p>
              </div>
            </div>
